...
public class CommonLogic
{
	...
	public static int getItemId(String name)
	{
		try 
		{
			ResultSet rs = CommonMethods.selectQuery("select ID from ItemMaster where ItemName='" + name + "'");
			rs.next();
			return rs.getInt(1);
		} catch (Exception e) 
		{
...
ResultSet rs = CommonMethods.selectQuery(query);
	...
	public static String getItemUsingCode(int code)
	{
		try {
			ResultSet rs = CommonMethods.selectQuery("select ItemName from ItemMaster where ItemCode="+code);
			if(rs.next())
			//rs.next();
			return rs.getString(1);
			else
			...
			System.out.println("Error in Getting Item Name "+e.getMessage());
			return "";
		}
	}
...
//String query ="select BankName from BankDetails order by BankName"; 			
    ...
    public static String getBankNameUsingId(int id)
    {
    	try {
			ResultSet rs = CommonMethods.selectQuery("select BankName from bankdetails where id="+id);
			rs.next();
			return rs.getString(1);
		} catch (Exception e) 
    	{
		System.out.println("Error getting Bank Name"+e.getMessage());
		return "";
		}
    
    }
    public static String getChequeNo(int id)
    {
    	try {
			ResultSet rs = CommonMethods.selectQuery("select ChequeNo from cashrecieved where id="+id);
			rs.next();
			return rs.getString(1);
		} catch (Exception e) 
    	{
    ...
    public static String getBankAccountNoUsingId(int id)
    {
    	try {
			ResultSet rs = CommonMethods.selectQuery("select AccountNo from bankdetails where id="+id);
			rs.next();
			return rs.getString(1);
		} catch (Exception e) 
    	{
    ...
    public static int getEmployeeId(String name)
    {
    	try {
    		String n[] = name.split(" ");
			rs = CommonMethods.selectQuery("select ID from employee "
					+ "where FName='"+n[0]+"' AND "
							+ "MName='"+n[1]+"' AND "
					+"LName='"+n[2]+"'");
			rs.next();
			...
			System.out.println("Error in getEmployeeId "+e.getMessage());
			return 0;
		}
    }
    public static List<String> getPartyNames()
    {
    	try {
			List<String> list = new ArrayList<>();
			rs = CommonMethods.selectQuery("select Name from purchaseparty order by Name");
			while(rs.next())
			{
				list.add(rs.getString(1));
			}
		...
		System.out.println("Error in Gettin Party Names "+e.getMessage());
		return null;
		}
    }
    public static List<String>getItemForPurchase()
    {
    	try {
    		List<String>list = new ArrayList<>();
			rs = CommonMethods.selectQuery("select ItemMaster.ItemName from ItemMaster,CategoryMaster where CategoryMaster.Stock='Y' And ItemMaster.CatId=categorymaster.ID");
		while(rs.next())
		{
			list.add(rs.getString(1));
		}
		...
		System.out.println("Error in Getting Item For Purchase "+e.getMessage());
		return null;
		}
    }
    ...
    {
    	try {
			rs =  CommonMethods.selectQuery("select Id from purchaseparty where name='"+name+"'");
			rs.next();
			return rs.getInt(1);
		} catch (Exception e) {
			System.out.println("Error in Getting party No"+e.getMessage());
			return 0;
		}
    }
    public static int reduceStock(String name,float qty)
    {
    	try {
    		rs = CommonMethods.selectQuery("select SIGN(stock),id from itemstock where ItemName='"+name+"'");
    		if(rs.next())
    		{
    			int flag=rs.getInt(1);
    			//System.out.println("Stock======>"+name+"====>"+rs.getInt(1));
    			if(flag==-1)// if stock is -
    			{
    				return 0;
    			}else
    			if(flag==1)//if stock is great than 0
    			{
    				return CommonMethods.updateRecord("update Itemstock set Stock=Stock-"+qty+" where ID="+rs.getInt(2));
    			}else if(flag==0)//if stock is 0
    			{
    				return 0;
    			}else return 0;
			...
			System.out.println("error in reducing Stock "+e.getMessage());
			return 0;
		}
    }
    public static int addStock(String name,float qty)
    {
    	try {
    		return CommonMethods.updateRecord("update ItemStock set Stock=Stock+"+qty+" where ItemName='"+name);
		} catch (Exception e) {
			System.out.println("Error in adding stock "+e.getMessage());
			return 0;
		}
    }
    public static Boolean getStockStatus(String name)
    {
    	try {
			rs = CommonMethods.selectQuery("select Stock from categorymaster where Category='"+name+"'");
			if(rs.next())
			{
			...
			System.out.println("error in Getting stock Status "+e.getMessage());
			return false;
		}
    }
    public static List<Passbook> getAllPassbook()
    {
			...
			try {
				...
				rs = CommonMethods.selectQuery("select "
						+ "ID, CID, Amount, Mode, TId, STR_TO_DATE(Date,'%d') from passbook order by ID");
				while(rs.next())
				{
					p = new Passbook();
					p.setID(rs.getInt(1));
					...
					p.setMode(rs.getString(4));
					p.setTId(rs.getInt(5));
    ...
    {
    	try {
			rs = CommonMethods.selectQuery("select Name from purchaseparty where Id="+id);
			if(rs.next())
			{
				return rs.getString(1);
			}
    ...
    public static List<PurchaseTransaction> getPurchaseTransacton(int billNo)
    {
    	try {
			List<PurchaseTransaction> list = new ArrayList<>();
			PurchaseTransaction t;
			rs = CommonMethods.selectQuery("select ID, ItemName, QTY, Rate, Amount, BillNo from purchasetransaction where BillNo="+billNo);
			while(rs.next())
			{
				t = new PurchaseTransaction();
				t.setId(rs.getInt(1));
				t.setItemName(rs.getString(2));
				t.setQty(rs.getInt(3));
				t.setRate(rs.getFloat(4));
				t.setAmount(rs.getFloat(5));
				t.setBillNo(rs.getInt(6));
				list.add(t);
				t=null;
    ...
    {
    	try {
			rs = CommonMethods.selectQuery("select id, TableName, DESCRIPTION from TableMaster");
			Table t;
			...
			System.out.println("Error in getting table list"+e.getMessage());
			return null;
		}
    }

	public static double getAvailableCash(int bankId)
    {
    	try {
			...
			System.out.println("Error in Getting Available Cash");
			return -1;
		}
    }

	public static String getBankAccountType(int bankId) {
		try {
			rs = CommonMethods.selectQuery("select AccountType from BankDetails where id=" + bankId);
			if (rs.next()) {
				return rs.getString(1);
			} else
				return "";
		} catch (Exception e) {
			System.out.println("Error in Gettin Account Type " + e.getMessage());
			return "";
		}
	}

	public static String getBankAccountIFSC(int bankId) {
		try {
			rs = CommonMethods.selectQuery("select IFC from BankDetails where id=" + bankId);
			if (rs.next()) {
				return rs.getString(1);
			} else
				return "";
	...
	public static String getBankAccountHolderName(int bankId) {
		try {
			rs = CommonMethods.selectQuery("select PersonName from BankDetails where id=" + bankId);
			if (rs.next()) {
				return rs.getString(1);
			} else
				return "";
	...
	public static String getemployeeFullName(int id) {
		try {
			rs = CommonMethods.selectQuery("select FNAME, MNAME, LNAME from Employee where id=" + id);
			return (rs.next() ? rs.getString(1) + " " + rs.getString(2) + " " + rs.getString(3) : "");
		} catch (Exception e) {
			System.out.println("Error in getting employee Full Name " + e.getMessage());
			return "";
		}
	}

	public static float getEmployeeDefSalary(int id) {
		try {
			rs = CommonMethods.selectQuery("select salary from employee where id=" + id);
			return (rs.next() ? rs.getFloat(1) : 0.0f);
		} catch (Exception e) {
			System.out.println("Error in getting Employee Default salary " + e.getMessage());
			return 0.0f;
		}
	}

	public static float getEmployeeAttendance(int month, int eid, int year) {
		try {
			float day=0;
			rs = CommonMethods.selectQuery("SELECT attendance FROM employeeattendance " + "where Year(date)=" + year
					+ " " + "And Month(date)=" + month + " And EmpId=" + eid);
			while (rs.next())
			{
				if(rs.getString(1).equals("AaQaa- idvasa"))
				{
					day=day+0.5f;
				}
				else if(rs.getString(1).equals("hjar"))
				{
					day=day+1.0f;
				}
				else 
	...
	public static String getCategoryName(int id) {
		try {
			rs = CommonMethods.selectQuery("select Category from categorymaster where Id=" + id);
			if (rs.next())
				return rs.getString(1);
			else
				return "";
		...
		{
			try {
				rs = CommonMethods.selectQuery("select ItemName from Itemmaster where Id="+id);
				if(rs.next())
				{
					return rs.getString(1);
				}
