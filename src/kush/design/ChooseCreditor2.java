...
public class ChooseCreditor2 extends JFrame
{
	...
	String date;
	public ChooseCreditor2(final int BillNo,final String date)
	{
		this.date=date;
		...
		jlist.addKeyListener(new KeyAdapter() {
			public void keyTyped(KeyEvent ke)
			{
				...
				if(c==KeyEvent.VK_ENTER)
				{
					...
					txtNewremaining.setText(""+(Float.parseFloat(txtRemaining.getText())+Float.parseFloat(txtNewcredit.getText())));
	...
	public void save(int billno,String date)
	{
		try {
			...
			int b=0;
			ResultSet r = CommonMethods.selectQuery("select Id from Passbook where Tid="+billno+" and Mode='Credit'");
			while(r.next())
			{
				b=r.getInt(1);
			}
			...
			if(b>0)
			{
				...
				int j = CommonMethods.updateRecord("update Passbook set CID="+id+",Amount="+Float.parseFloat(txtNewcredit.getText())+", TID="+billno+" where id="+b);
				int f = CommonMethods.updateRecord("update Bill set PayMode='Credit', CustomerId="+id+",Status='Paid',BillDate="+date+" where Billno="+billno);
				if(j==1 && f==1)
				{
					System.out.println("Bill to print "+billno);
					new BillPdf(billno);
					JOptionPane.showMessageDialog(this,"Record Update Success","Success",JOptionPane.INFORMATION_MESSAGE);
			...
			int f = CommonMethods.updateRecord("update Bill set PayMode='Credit', CustomerId="+id+",Status='Paid' where Billno="+billno);
			int tit = CommonMethods.getId("select ID from Passbook order by ID");
			String query = "insert into passbook(id,CID,Amount,Mode,TId,Date) values("+tit+","+id+","+Float.parseFloat(txtNewcredit.getText())+",'Credit',"+billno+",CURDATE())";
