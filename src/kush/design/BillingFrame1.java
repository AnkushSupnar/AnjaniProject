...
public class BillingFrame1 extends JFrame implements ActionListener {
	...
	final int user;
	final static Logger logger = Logger.getLogger(BillingFrame1.class);

	public BillingFrame1(final int user) {
		setIconImage(Toolkit.getDefaultToolkit().getImage(BillingFrame1.class.getResource("/kush/images/Logo.jpg")));
		this.user = user;
		...
		scrollPane_1.setBounds(425, 560, 629, 148);
		...
		tabbedPane.setBounds(1056, 0, 208, 708);
		...
		btnoldbill.setBounds(724, 470, 109, 40);
	...
	void findItemFilter(String find) {
		...
		try {
			for (int i = 0; i < listItem.size(); i++) {
				if (((String) listItem.get(i)).startsWith(find)) {
					// MySortStrings.add(MyStrings.get(i));
					itemlistmodel.addElement((String) listItem.get(i));
				}
			}
		} catch (Exception e) {
			System.out.println("Error in findItem " + e.getMessage());
	...
	void addButton() {
		...
		if (Float.parseFloat(txtQty.getText()) < 0.0f) {
			/*
			 * //System.out.println("Minus Stop Here"); String itemname = txtItem.getText();
			 * int qty = Integer.parseInt(txtQty.getText()); float rate =
			 * Float.parseFloat(txtRate.getText());
			 * //System.out.println("Name "+itemname+" QTY "+qty+" Rate"+rate);
			 */
	...
	void close(int user) {
		try {
			...
			if (bill == 0)// not in bill table i have add fresh
			{
				...
				// Bill(BILLNO,BILLAMT,DISCOUNT,CUSTOMERID,WAITORID,TABLENO,USERID,BILLDATE,BILLTIME,PAYMODE,STATUS)
				// values("
