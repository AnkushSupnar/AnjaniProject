...
public class Passbook extends JFrame {
	 ...
	 * 
	 */
	private static final long serialVersionUID = 7206124633350302578L;
	...
	public Passbook() {
		...
		setTitle("All Payment Recived Report");
		...
		txtId.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent e) {
				...
				if (!((ch >= '0') && (ch <= '9') || (ch == '.') || (ch == '-') || (ch == KeyEvent.VK_BACK_SPACE)
						|| (ch == KeyEvent.VK_ENTER) || (ch == KeyEvent.VK_DELETE))) {
					// getToolkit().beep();
					e.consume();
				}
		...
		btnPrint.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if ((new File("D:\\Hotel Software\\Passbook\\bill.pdf")).exists()) {

						Process p = Runtime.getRuntime()
								.exec("rundll32 url.dll,FileProtocolHandler D:\\Hotel Software\\Passbook\\bill.pdf");
						p.waitFor();

					} else {

						System.out.println("File is not exists");
						JOptionPane.showMessageDialog(null, "File Not Found Please Generate PDF", "Error",
		...
		txtCustomername.addKeyListener(new KeyAdapter() {
			...
			public void keyTyped(KeyEvent ke) {
				...
				// System.out.println("Enter name"+txtCustomername.getText());
				if (!list.isEmpty()) {
	...
	public void GetAllDates() {
		try {
			...
			ResultSet rs = CommonMethods.selectQuery(
					"select BillDate ,BillAmt+Discount from Bill where CustomerID=" + 1 + " and Paymode='credit'");
	...
	public void getAllData(int id) {
		try {
			...
			while (rs.next()) {
				...
				// tmodel.addRow(new Object[] {++i,"jamaa paavataI
				// k`"+rs.getInt(5),rs.getDate(6),"",rs.getFloat(3)});
